...
public class GerenciarServico implements Serializable {
    ...
    public boolean editarOrdem(int id, String novaDescricao, String novoStatus) {
        for (int i = 0; i < ordens.size(); i++) {
            ...
            if (os.getId() == id) {
                OrdemServico novaOrdem = new OrdemServico(id, novaDescricao, os.getVeiculo());
                novaOrdem.getPecas().addAll(os.getPecas());
                if (novoStatus != null && novoStatus.equalsIgnoreCase("Finalizada")) {
                    novaOrdem.finalizarOrdem();
                }
                ordens.set(i, novaOrdem);
                System.out.println("Ordem atualizada: " + novaOrdem.toString());
                return true;
            }
        }
